...
const CBA_GYM_NAME = 'Ginásio da Vila Militar - Dendezeiros';
const CBA_GYM_MAP_URL = 'https://www.google.com/maps/place/Academia+de+Pol%C3%ADcia+Militar+da+Bahia/@-12.9301372,-38.5094415,130m/data=!3m1!1e3!4m10!1m2!2m1!1svila+militar+dendezeiros!3m6!1s0x71605929528a4bb:0xa5a7cd48de968379!8m2!3d-12.9297899!4d-38.5092146!15sChh2aWxhIG1pbGl0YXIgZGVuZGV6ZWlyb3OSARtkZXBhcnRtZW50X29mX3B1YmxpY19zYWZldHngAQA!16s%2Fg%2F11fhky1q2k?entry=ttu&g_ep=EgoyMDI2MDkyMS4wIKXMDSoASAFQAw%3D%3D';
const isCbaGym = location => /vila militar|dendezeiros|academia de pol[ií]cia militar/i.test(String(location||''));
function GameLocationLink({ location }) {
  if (!isCbaGym(location)) return null;
  return <a href={CBA_GYM_MAP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 min-h-10 text-xs font-black text-emerald-300 underline underline-offset-4 hover:text-emerald-200" aria-label="Abrir localização do Ginásio da Vila Militar no Google Maps"><MapPin className="w-4 h-4"/>Abrir no Google Maps ↗</a>;
}

function GameForm({ item, onClose, onSaved }) {
  const [form,setForm]=useState({date:item?.date||'',time:item?.time||'',location:item?.location||CBA_GYM_NAME});
  const [busy,setBusy]=useState(false);
...
function GamesView({ data, refresh }) {
  ...
  return <div className="space-y-5 pb-24 md:pb-8">
    ...
    {next && <Panel className="p-5 border-emerald-500/30"><div className="flex flex-col md:flex-row md:items-center gap-5 justify-between"><div><Pill tone="emerald">Próximo jogo</Pill><h3 className="text-2xl font-black text-white mt-3">{fmtDate(next.date)} · {next.time}</h3><p className="text-sm text-slate-400 mt-1 flex items-center gap-2"><MapPin className="w-4 h-4"/>{next.location}</p><GameLocationLink location={next.location}/><p className="text-xs text-slate-500 mt-3">{next.confirmed.length} confirmados</p></div><button disabled={busy} onClick={()=>attend(next)} className={cx('rounded-2xl px-6 py-3.5 font-black',next.confirmed.includes(ownName)?'bg-rose-500/10 border border-rose-500/30 text-rose-300':'bg-emerald-500 text-slate-950')}>{next.confirmed.includes(ownName)?'Desistir':'Estou dentro'}</button></div></Panel>}

...
export default function AdminDashboardBridge() {
  ...
  return <>{mountNode&&createPortal(<button title="Administração" onClick={()=>setOpen(true)} className="flex items-center justify-center w-12 h-12 rounded-2xl transition-all duration-300 text-indigo-400 hover:bg-indigo-950/60 border border-indigo-500/20"><ShieldCheck className="w-6 h-6 md:w-7 md:h-7"/></button>,mountNode)}{open&&<AdminConsole onClose={()=>setOpen(false)}/>}</>;
}
